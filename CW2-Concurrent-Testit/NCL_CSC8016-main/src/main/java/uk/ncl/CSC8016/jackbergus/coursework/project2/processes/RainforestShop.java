...
public class RainforestShop {
     ...
     */
    protected boolean logout(Transaction transaction) {
        ...
        // TODO: Implement the remaining part!
            ...
            if (transaction != null && UUID_to_user.containsKey(transaction.getUuid()) && UUID_to_user.containsValue(transaction.getUsername())) {
                result = true;


                if (!available_withdrawn_products.isEmpty()) {


                    for (ProductMonitor p : available_withdrawn_products.values()) {
                        for (Item item : p.withdrawn) {
                            p.doShelf(item);
                        }
    ...
    List<String> getAvailableItems(Transaction transaction) {
        ...
        ProductMonitor p = new ProductMonitor();

                if (!p.withdrawn.contains(p.available)) {
                    if (!p.available.isEmpty())
                        ls.add(p.available.toString());
                }
    ...
    Optional<Item> basketProductByName(Transaction transaction, String name) {
        ...
        // TODO: Implement the remaining part!
        if (!available_withdrawn_products.containsKey(name)) {
            return result.get();
        }

        ProductMonitor p = available_withdrawn_products.get(name);
        result.set(p.getAvailableItem());
    ...
    boolean shelfProduct(Transaction transaction, Item object) {
        ...
        if (available_withdrawn_products.containsKey(object.productName)) {
            ProductMonitor p = available_withdrawn_products.get(object.productName);

            result = p.doShelf(object);
